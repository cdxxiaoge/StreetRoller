...
class Mymqtt(): 
    def __init__(self, name: str) -> None:
        ...
        self.IsRun = 0
        # 小车当前位置
        self.current_pos = [1*self.ratio, Coordinate_trans(0*self.ratio)]
        # 小车目标位置
        self.target_pos = [1*self.ratio, Coordinate_trans(0*self.ratio)]
        # 当前速度
        self.current_vel = 0
        # 小车匀速时速度
        self.target_vel = 0
        ...
        self.state = 1
        # 小车最大速度
        self.Maxvel = 1.5 * self.ratio
        # 小车的正向加速度
        self.acceler_pos = 0.1 * self.ratio
        # 小车的负向加速度
        self.acceler_ne = 0.15 * self.ratio
        # 1、当前距目标点距离、2、到目标点所剩时间
        self.target_message = [0, 0]
        # 是否倒车,1代表倒车
        self.backoff = 0
        # 小车角度
        self.angle = 0
    ...
    # 设置速度上限和下限
    def sat(self):
        if self.target_vel > self.Maxvel:
            self.target_vel = self.Maxvel
